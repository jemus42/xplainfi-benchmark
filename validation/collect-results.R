...
# results/validation/. A frozen reference table produced by an earlier "all" or
# "reference"-only run is retained and reused (see combine below).
for (prov_name in unique(collected$provider)) {
	save_reduced(collected[provider == prov_name], lane = "validation")
}
...
reference_frozen <- latest_reduced("validation", "reference")
